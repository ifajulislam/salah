...
import { color, radius, skyGradients, spacing, type, SkyPeriod } from '@/theme/tokens';
...
export function HeroCard({ next, previous, skyPeriod, use24Hour, sunrise, sunset, now }: Props) {
  const countdown = useCountdown(next.date, previous.date);
  ...
  return (
    ...
    >
      ...
      <View style={styles.nameRow}>
        <Text style={styles.enName}>{next.label}</Text>
        <Text style={styles.arName}>{next.labelArabic}</Text>
      </View>
      <Text style={styles.time}>{formatTime(next.date, use24Hour)}</Text>

      <View style={styles.countdownRow}>
        <CountdownRing size={64} strokeWidth={6} progress={countdown.progress} />
        <Text style={styles.countdownLabel}>
          {countdown.label} <Text style={styles.countdownSub}>remaining</Text>
        </Text>
      </View>
